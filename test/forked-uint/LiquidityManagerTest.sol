...
contract LiquidityManagerTest is Test {
    ...
    uint256 constant SMALL_SWAP_AMOUNT = 10 * 1e6; // 10 USDC (6 decimals)

    // Known Uniswap V3 USDC/WETH pool address on Base Sepolia
    address public constant USDC_WETH_POOL = 0x94bfc0574FF48E92cE43d495376C477B1d0EEeC0;
    ...
    function testSwapOnUniswap() public {
        ...
        //     address pool =
        //         IUniswapV3Factory(networkConfig.uniswapFactory).getPool(networkConfig.usdc, networkConfig.weth, FEE_TIER);
        ...
        //     // Initial balances
        //     uint256 initialUsdcBalance = IERC20(networkConfig.usdc).balanceOf(networkConfig.vault);
        //     uint256 initialWethBalance = IERC20(networkConfig.weth).balanceOf(networkConfig.vault);
        ...
        //     // Execute swap
        //     uint256 amountOut =
        //         supplyLiquidity.swapOnUniswap(networkConfig.usdc, networkConfig.weth, amountIn, amountOutMin, FEE_TIER);

        //     // Final balances
        //     uint256 finalUsdcBalance = IERC20(networkConfig.usdc).balanceOf(networkConfig.vault);
        //     uint256 finalWethBalance = IERC20(networkConfig.weth).balanceOf(networkConfig.vault);

        //     // Assertions
        //     assertGt(amountOut, 0, "Swap should return non-zero amount");
        //     assertEq(initialUsdcBalance - finalUsdcBalance, amountIn, "Incorrect USDC deducted");
        //     assertGt(finalWethBalance, initialWethBalance, "Vault should receive WETH");
    ...
    function test_Revert_SwapOnUniswapInsufficientAllowance() public {
        ...
        vm.expectRevert();
        supplyLiquidity.swapOnUniswap(networkConfig.usdc, networkConfig.weth, amountIn, 0, FEE_TIER);
        vm.stopPrank();
    }
    ...
    function testAddLiquidityToPool() public {
        ...
        //     // Approve both tokens
        //     IERC20(networkConfig.usdc).approve(address(supplyLiquidity), amount0Desired);
        //     IERC20(networkConfig.weth).approve(address(supplyLiquidity), amount1Desired);
        ...
        //         networkConfig.usdc, networkConfig.weth, amount0Desired, amount1Desired, FEE_TIER, tickLower, tickUpper
        //     );
    ...
    function testSupplyUSDCOnAave() public {
        ...
        (uint256 totalCollateralBase,,,,,) = supplyLiquidity.getAaveLiquidityStatus(owner);
    ...
    function testWithdrawUSDCFromAave() public {
        ...
        supplyLiquidity.withdrawLiquidityFromAave(networkConfig.usdc, networkConfig.aaveUsdc, 69);
        (uint256 totalCollateralBase,,,,,) = supplyLiquidity.getAaveLiquidityStatus(owner);
    ...
    function testWithdrawUSDCFromCompound() public {
        ...
        IERC20(networkConfig.usdc).approve(address(supplyLiquidity), 69000000);
        supplyLiquidity.supplyLiquidityOnCompound(networkConfig.usdc, 69000000);
        CometExtInterface(networkConfig.compoundUsdc).allow(address(supplyLiquidity), true);
